            ...
            if (this.state.players.get(client.sessionId)) {
                ...
                for (var m = 0; m < col.length; m++) {
                    ...
                    if (b[col[m]] == this.state.players.get(client.sessionId).dice && col[m] == message.i - 1) {
                        ...
                        this.state.players.get(client.sessionId).history = new schema_1.ArraySchema().concat(h);
        ...
        this.onMessage("magic", (client, message) => {
            ...
            if (this.state.players.get(client.sessionId)) {
                ...
                if (lr > 0) {
                    ...
                    this.state.players.get(client.sessionId).history = new schema_1.ArraySchema();
    ...
    */
    ...
    addLines(c, l = 1) {
        if (this.state.players.get(c.sessionId)) {
            ...
            for (var i = 0; i < l; i++) {
                let line = new schema_1.ArraySchema();
...
GameRoom.DROP_TIMEOUT = 8;
GameRoom.MAGIC_TIMEOUT = 2;
GameRoom.WIN_TIMEOUT = 1;
GameRoom.BOARD_WIDTH = 6;
GameRoom.BOARD_HEIGHT = 10;
GameRoom.PLAYER_NUM = 2;
GameRoom.INIT_LINES = 2;
GameRoom.DICE_NUM = 6; // 1~6
